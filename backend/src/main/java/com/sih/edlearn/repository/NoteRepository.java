...
public interface NoteRepository extends JpaRepository<Note, Long> {
    ...
    Page<Note> findByTeacherIdAndStatus(Long teacherId, Note.Status status, Pageable pageable);
     ...
     @Query("SELECT DISTINCT n FROM Note n " +
            ...
            "LEFT JOIN FETCH n.subject " +
            "LEFT JOIN FETCH n.schoolClass " +
            "WHERE n.schoolClass.id = :classId AND n.subject.id = :subjectId AND n.chapter.id = :chapterId AND n.status = :status " +
            "ORDER BY n.createdAt DESC")
     Page<Note> findBySchoolClassIdAndSubjectIdAndChapterIdAndStatus(
             Integer classId, Integer subjectId, Integer chapterId, Note.Status status, Pageable pageable);
}
